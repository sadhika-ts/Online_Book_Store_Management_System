...
{
    public partial class RemoveBooks : System.Web.UI.Page
    {
        ...
        protected void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            if (currentStocks != 0)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Cannot remove book. Stock must be 0.');", true);
                return;
            }
            ...
            if (rbDiscounts.Checked)
            {
                query = "DELETE FROM addbooks WHERE bookid = @BookID and category = 'discounts'";
            }
            else if (rbNewArrivals.Checked)
            {
                query = "DELETE FROM addbooks WHERE bookid = @BookID and category = 'newarrivals'";
            }
            else if (rbNormalBooks.Checked)
            {
                query = "DELETE FROM addbooks WHERE bookid = @BookID and category = 'normalbook'";
            }
            ...
            if (!string.IsNullOrEmpty(query))
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Book removed successfully!');", true);
                    ...
                    {
                        // Show error message if no rows were affected (e.g., BookID not found)
                        ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Book not found or could not be removed.');", true);
                    }
                }
            }
